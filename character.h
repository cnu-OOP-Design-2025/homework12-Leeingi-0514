...
public:
    Knight() { description = "Knight"; type = CharacterType::Knight; auto logger = Logger::getInstance(); logger->log("[Create] Knight");
    }
...
public:
    Wizard() { description = "Wizard"; type = CharacterType::Wizard; auto logger = Logger::getInstance(); logger->log("[Create] Wizard");
    }
...
public:
    Archer() { description = "Archer"; type = CharacterType::Archer;auto logger = Logger::getInstance(); logger->log("[Create] Archer");
    }
...
public:
    Armor(shared_ptr<Character> c) : EquipDeco(c, "Armor") {
        auto logger = Logger::getInstance(); logger->log("[Trying to Equip] " + getDescription());
    } 
...
public:
    Staff(shared_ptr<Character> c) : EquipDeco(c, "Staff") {
        auto logger = Logger::getInstance(); logger->log("[Trying to Equip] " + getDescription());
        if(!(c->getType() == CharacterType::Wizard)) throw "Staff requires Wizard";
    }
...
public:
    Bow(shared_ptr<Character> c) : EquipDeco(c, "Bow") {
        auto logger = Logger::getInstance(); logger->log("[Trying to Equip] " + getDescription());
        if(c->getType() == CharacterType::Wizard) throw "Bow requires Archer or Knight";
    }
